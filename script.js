...
function calculateHoursBefore() {
  ...
  if (workTime == 0 && workRe == 0 && oneTimeWork == 0 && pjt == 0 && otherWork == 0) {
    ...
    if (monthHourBefore > 0 && monthHourAfter > 0) {
      document.getElementById("result").innerText = ["改善前：", monthHourBefore, workTimeStr, "/月 - 改善後：", monthHourAfter, workTimeStr, "/月 = ", Math.round((monthHourBefore - monthHourAfter) * 100) / 100, workTimeStr, "/月の工数低減"].join("");
      if (workTimeStr === "h") {
        ...
        document.getElementById("resultPoint").innerText = "";
      }
    }
  }
}
